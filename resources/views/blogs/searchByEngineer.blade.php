...
<div class="container">
    <div class="row">
        ...
        <div class="col-lg-12">
            ...
            <div class="row">
                @if ($engineerTasks->count() == 0 )
                <h2 class="text-danger text-center">There is No data </h2>
                @endif
                @foreach($engineerTasks as $engineer)

                <div class="col-lg-4">
                    <!-- Blog post-->
                    <div class="card mb-4">
                        <div class="card-body">
                            <div class="small text-muted">Date: {{$engineer->report_date}}</div>
                            <h6 class="card-title  ">Station :{{$engineer->ssname}}</h6>
                            <h6 class="card-title">Engineer : {{$engineer->eng_name}}</h6>
                            <p class="card-text text-danger">Nature of fault : {{$engineer->problem}}</p>
                            <p class="card-text text-success">Action take: {{$engineer->action_take}}
                                {{$engineer->reason}}</p>
                            <a class="btn btn-primary" href="{{route('blogs.details',['id'=>$engineer->id_task])}}">Read
                                more →</a>
                        </div>
                    </div>
                </div>
